...
class TestCase(unittest.TestCase) :
    ...
    def testReadGromosTraj(self):
        fileN = os.path.join(RDConfig.RDBaseDir,'Contrib','ConformerParser',
                                            'test_data','water_coords.trc')
        mol = Chem.MolFromSmiles('O')
        mol = Chem.AddHs(mol)
        ids = rdConformerParser.AddConformersFromGromosTrajectory(mol, fileN)
        self.failUnless(mol.GetNumConformers() == 1)
        self.failUnless(len(ids) == 1)
        self.failUnless(ids[0] == 0)
        
        fileN = os.path.join(RDConfig.RDBaseDir,'Contrib','ConformerParser',
                                              'test_data','water_coords2.trc')
        ids = rdConformerParser.AddConformersFromGromosTrajectory(mol, fileN, clearConfs=True)
        self.failUnless(mol.GetNumConformers() == 2)
        ids = rdConformerParser.AddConformersFromGromosTrajectory(mol, fileN, clearConfs=False)
        self.failUnless(mol.GetNumConformers() == 4)
        ids = rdConformerParser.AddConformersFromGromosTrajectory(mol, fileN, numConfs=1, clearConfs=True)
        self.failUnless(mol.GetNumConformers() == 1)
