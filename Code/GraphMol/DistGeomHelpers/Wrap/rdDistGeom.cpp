...
namespace RDKit {
  ...
  INT_VECT KnowledgeEmbedMultipleConfs(ROMol &mol, unsigned int numConfs,
                              unsigned int maxAttempts,
                              int seed, bool clearConfs,
                              DGeomHelpers::ExpTorsionLevel torsionAnglePrefLevel,
                              double boxSizeMult, bool randNegEig,
                              unsigned int numZeroFail, double pruneRmsThresh,
                              double forceTol,
                              bool ignoreSmoothingFailures) {

    INT_VECT res = DGeomHelpers::KnowledgeEmbedMultipleConfs(mol, numConfs, maxAttempts,
                                                             seed, clearConfs,
                                                             torsionAnglePrefLevel, boxSizeMult,
                                                             randNegEig, numZeroFail,
                                                             pruneRmsThresh,forceTol,
                                                             ignoreSmoothingFailures);
  ...
  PyObject *getMolMultiRangeBoundsMatrix(ROMol &mol, bool set15bounds=true,
                                         bool scaleVDW=false,
                                         DGeomHelpers::ExpTorsionLevel torsionAnglePrefLevel=DGeomHelpers::TOLERANCE1) {
    unsigned int nats = mol.getNumAtoms();
    npy_intp dims[3];
    dims[0] = nats;
    dims[1] = nats;
...
\n";
  ...
  docString = "Use distance geometry to obtain multiple sets of \n\
   coordinates for a molecule based on experimental torsion angle preferences\n\
   \n\
   ARGUMENTS:\n\n\
    - mol : the molecule of interest\n\
    - numConfs : the number of conformers to generate \n\
    ...
    - randomSeed : provide a seed for the random number generator \n\
                   ...
                   (-1) uses a random seed \n\
    - clearConfs : clear all existing conformations on the molecule\n\
    - torsionAnglePrefLevel : level for the experimental torsion angles\n\
    - boxSizeMult    Determines the size of the box that is used for\n\
                     random coordinates. If this is a positive number, the \n\
                     side length will equal the largest element of the distance\n\
                     matrix times boxSizeMult. If this is a negative number,\n\
   ...
   RETURNS:\n\n\
      List of new conformation IDs \n\
  \n";
    python::def("KnowledgeEmbedMultipleConfs", RDKit::KnowledgeEmbedMultipleConfs,
                (python::arg("mol"), python::arg("numConfs")=10,
                 python::arg("maxAttempts")=0,
                 python::arg("randomSeed")=-1, python::arg("clearConfs")=true,
                 python::arg("torsionAnglePrefLevel")=RDKit::DGeomHelpers::TOLERANCE1,
                 python::arg("boxSizeMult")=2.0,
                 python::arg("randNegEig")=true, python::arg("numZeroFail")=1,
                 python::arg("pruneRmsThresh")=-1.0,
                 python::arg("forceTol")=1e-3,
...
\n";
  ...
  docString = "Returns the multi-range bounds matrix for a molecule\n\
   \n\
   ARGUMENTS:\n\n\
      - mol : the molecule of interest\n\
      - set15bounds : set bounds for 1-5 atom distances based on \n\
   ...
   RETURNS:\n\n\
      ...
      the lower triangle and upper bounds in the upper triangle\n\
  \n";
    python::def("GetMoleculeMultiRangeBoundsMatrix", RDKit::getMolMultiRangeBoundsMatrix,
                (python::arg("mol"), python::arg("set15bounds")=true,
                 python::arg("scaleVDW")=false,
                 python::arg("torsionAnglePrefLevel")=RDKit::DGeomHelpers::TOLERANCE1),
                docString.c_str());
