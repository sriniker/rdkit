...
/*!
  ...
  \param boxSizeMult    Determines the size of the box that is used for
                        ...
                        negative number, the side length will equal \c -boxSizeMult
  ...
  \param coordMap  a map of int to Point3D, between atom IDs and their locations
                   ...
                   coordinates are used to set distance constraints on the embedding.
                   The resulting conformer(s) should have distances between the
                   specified atoms that reproduce those between the points in \c
                   coordMap. Because the embedding produces a molecule in an
                   arbitrary reference frame, an alignment step is required to
                   actually reproduce the provided coordinates.
  ...
  \param ETVersion      version of the experimental torsion preferences.
                        There are two versions (default: version 1).
...
int EmbedMolecule(ROMol &mol, unsigned int maxIterations = 0, int seed = -1,
                  ...
                  int ETversion = 1, double basinThresh = 5.0);
